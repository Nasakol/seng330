...
public interface ContactRepository extends Repository<Contact, Integer> {
    ...
    @Transactional(readOnly = true)
    Contact findById(Integer contactId) throws DataAccessException;
    
//    @Query("SELECT DISTINCT contact FROM Contact c WHERE contact.type=:type")
    @Transactional(readOnly = true)
    Collection<Contact> findByType(String type) throws DataAccessException;
    
//    @Query("SELECT DISTINCT contact FROM Contact c WHERE contact.name LIKE :name%")
    @Transactional(readOnly = true)
